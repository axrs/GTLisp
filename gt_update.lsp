(DEFUN gtupdate:update (/ $xml)
  ...
  ;;
  ;;=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-;;
  (DEFUN gtupdate:download (/ $url cp ok tmp util)
	...
	;;(SETQ $url "http://software.glynntucker.com.au/GTLISP/0.0.10/GTLISP.VLX")
	(PRINC
	...
	(IF	(VL-CATCH-ALL-ERROR-P
		  (VL-CATCH-ALL-APPLY
			'VL-FILE-COPY
			(LIST (STRCAT (gtconfig:getvalue $gtconfig "GENERAL" "BASEPATH")
						  "\\GTLISP.VLX"
				  )
				  (STRCAT (gtconfig:getvalue $gtconfig "GENERAL" "BASEPATH")
						  "\\GTLISP.VLX.bak"
	...
	(SETQ cp (STRCAT (gtconfig:getvalue $gtconfig "GENERAL" "BASEPATH")
					 "\\"
